...
def create_cgroup(user, group, interface, gw=None,
                  gw_6=None, default_int=None):
    ...
    cgroup_iptables = [
        ...
        ["-t", "nat", "-A", "POSTROUTING", "-m", "cgroup", "--cgroup",
         "0x00110011", "-o", "{}".format(interface),
         "-j", "MASQUERADE"],
        ["-I", "OUTPUT", "1", "-m", "cgroup", "--cgroup", "0x00110011", "-j",
         "ACCEPT"],
        ["-I", "INPUT", "1", "-m", "cgroup", "--cgroup", "0x00110011", "-j",
         "ACCEPT"],
        ["-t", "nat", "-A", "OUTPUT", "-m", "cgroup", "--cgroup", "0x00110011",
         "-p", "tcp", "--dport", "53", "-j", "REDIRECT", "--to-ports", "5354"],
        ["-t", "nat", "-A", "OUTPUT", "-m", "cgroup", "--cgroup", "0x00110011",
    ...
    try:
        check_call(["ip", "rule", "add", "fwmark",
                    "11", "table", "bypass_qomui"])
        if interface == default_int:
            check_call(["ip", "route", "flush", "table", "bypass_qomui"])
            check_call(["ip", "rule", "add", "fwmark",
                        "11", "table", "bypass_qomui"])
            check_call(["ip", "route", "add", "default", "via", "{}".format(
                gw), "dev", "{}".format(interface), "table", "bypass_qomui"])
            logging.debug(
    ...
    try:
        check_call(["ip", "-6", "rule", "add", "fwmark",
                    "11", "table", "bypass_qomui"])
        if interface == default_int:
    ...
    try:
        ...
        logging.error(
            "Failed to disable reverse path filtering for {}".format(interface))

    try:
        check_call(["cgcreate", "-t", "{}:{}".format(user, group), "-a"
...
def delete_cgroup(interface):
    ...
    cgroup_iptables_del = [
        ["-t", "mangle", "-D", "OUTPUT", "-m", "cgroup", "--cgroup",
         "0x00110011", "-j", "MARK", "--set-mark", "11"],
        ["-t", "nat", "-D", "POSTROUTING", "-m", "cgroup", "--cgroup",
         "0x00110011", "-o", "{}".format(interface),
         "-j", "MASQUERADE"],
        ["-D", "OUTPUT", "-m", "cgroup", "--cgroup", "0x00110011", "-j",
         "ACCEPT"],
        ["-D", "INPUT", "-m", "cgroup", "--cgroup", "0x00110011", "-j",
         "ACCEPT"],
        ["-t", "nat", "-D", "OUTPUT", "-m", "cgroup", "--cgroup", "0x00110011",
         "-p", "tcp", "--dport", "53", "-j", "REDIRECT", "--to-ports", "5354"],
        ["-t", "nat", "-D", "OUTPUT", "-m", "cgroup", "--cgroup", "0x00110011",
         "-p", "udp", "--dport", "53", "-j", "REDIRECT", "--to-ports", "5354"]]

    try:
        check_call(["ip", "rule", "del", "fwmark",
                    "11", "table", "bypass_qomui"])
    except CalledProcessError:
        pass
    ...
    try:
        ...
        logging.debug(
            "Bypass: Could not delete {} - resource does not exist or is busy".format(cgroup_path))
