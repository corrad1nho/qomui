...
def create_cgroup(user, group, default_interface, default_gateway, default_interface_6, default_gateway_6):
    ...
    cgroup_iptables = [
        ...
        ["-t", "nat", "-A", "POSTROUTING", "-m", "cgroup",
         "--cgroup", "0x00110011", "-o", "{}".format(default_interface), "-j", "MASQUERADE"],
        ["-I", "OUTPUT", "1", "-m", "cgroup",
    ...
    if default_gateway_6 != "None":
        ...
        cgroup_iptables.insert(1, ["-t", "nat", "-A", "POSTROUTING", "-m", "cgroup", "--cgroup",
                                   "0x00110011", "-o", "{}".format(default_interface_6), "-j", "MASQUERADE"])
    ...
    try:
        ...
        check_call(["ip", "route", "add", "default", "via",
                    "{}".format(default_gateway), "dev", "{}".format(default_interface), "table", "bypass_qomui"])
    except CalledProcessError:
    ...
    try:
        ...
        check_call(["ip", "-6", "route", "add", "default", "via",
                    "{}".format(default_gateway_6), "dev", "{}".format(default_interface_6), "table", "bypass_qomui"])
    except CalledProcessError:
    ...
    with open("/proc/sys/net/ipv4/conf/all/rp_filter", 'w') as rp_edit_all:
        rp_edit_all.write("2")
    with open("/proc/sys/net/ipv4/conf/{}/rp_filter".format(default_interface), 'w') as rp_edit_int:
        rp_edit_int.write("2")
...
def delete_cgroup(default_interface, default_interface_6):
    ...
    cgroup_iptables_del.insert(1, ["-t", "nat", "-D", "POSTROUTING", "-m", "cgroup",
                                   "--cgroup", "0x00110011", "-o", "{}".format(default_interface_6), "-j", "MASQUERADE"])
    ...
    try:
        ...
        logging.debug("Could not delete {} - resource does not exist or is busy".format(cgroup_path))
